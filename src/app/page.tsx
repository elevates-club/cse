...
export default function Home() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-primary">
       {/* Navigation Overlay */}
      <nav className="fixed top-0 left-0 w-full z-[80] px-4 md:px-8 py-4 md:py-6 flex justify-between items-center bg-primary/80 md:bg-transparent backdrop-blur-md md:backdrop-blur-none transition-all">
        <h3 className="font-display text-xl md:text-2xl tracking-tighter">CSE&apos;22-26</h3>
        <div className="flex items-center gap-3 md:gap-6">
          {isAuthenticated ? (
            <div className="flex items-center gap-3 md:gap-4">
               <span className="text-[10px] uppercase tracking-widest text-gold font-body hidden lg:block">
                Welcome, {user?.name.split(' ')[0]}
              </span>
              <button 
                onClick={handleUploadClick}
                className="px-4 md:px-5 py-2 bg-white text-primary text-[10px] md:text-xs font-bold rounded-full hover:bg-gold transition-all"
              >
                Upload
              </button>
              <button 
                onClick={logout}
                className="text-[9px] md:text-[10px] uppercase tracking-widest text-white/40 hover:text-white font-body transition-colors"
              >
                Sign Out
              </button>
            </div>
            ...
            <button 
              ...
              className="px-4 md:px-5 py-2 border border-white/10 text-white text-[10px] md:text-xs font-bold rounded-full hover:bg-white hover:text-primary transition-all"
            >
              Sign In
            </button>
          )}
        ...
        <div className="mb-8 md:mb-12 relative min-h-[300px] flex items-center justify-center">
          {heroMemory ? (
            <FujicamCard 
              ...
              index={0}
              rotation={0}
              quote="Where our stories are etched in digital dust."
